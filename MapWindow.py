...
def createnewemergencyPlan():
    ...
    def CreateNewCampScreen():
        ...
        def open_map():
            ...
            emergency_marker_label = Label(map_frame,
                                           text="Please right click the below map to select the country of the emergency")
            emergency_marker_label.pack()

            emergency_map = tkintermapview.TkinterMapView(map_frame, width=150, height=150, corner_radius=0)
            emergency_map.set_zoom(2)
            emergency_map.pack()
            emergency_map.add_right_click_menu_command(label="Emergency Marker", command=add_emergency_marker,
                                                       pass_coords=True)
            map_reset_button = Button(map_frame, text="Reset Map", command=mapReset)
            map_reset_button.pack()

            map_confirm = IntVar()
            map_confirm_entry = Button(map_frame, text="Confirm Map Entry", command=map_frame.destroy)
            map_confirm_entry.pack()

            map_frame.mainloop()

        location_button = Button(New_Camp_Screen, text="Choose Location", command=open_map)
