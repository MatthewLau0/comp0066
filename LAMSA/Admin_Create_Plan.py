...
def createnewemergencyPlan():
    ...
    emergency_type_entry = ttk.Combobox(New_Camp_Screen, textvariable=emergency_type, values=["Earthquake", "Tsunami", "Flood", "Drought", "Cyclone", "Hurricane", "Climate Change", "Food Shortage", "Health Epidemic", "War", "Industrial Accident", "Other: "])
    ...
    country_list = ['Afghanistan', 'Aland Islands', 'Albania', 'Algeria', 'American Samoa', 'Andorra', 'Angola',
                    ...
                    'Costa Rica', "Côte d'Ivoire", 'Croatia', 'Cuba', 'Curaçao', 'Cyprus', 'Czech Republic', 'Denmark',
                    ...
                    'Heard Island and McDonald Islands', 'Vatican City', 'Honduras', 'Hong Kong', 'Hungary', 'Iceland',
                    ...
                    'Russia', 'Rwanda', 'Saint Lucia', 'Samoa', 'San Marino', 'Sao Tome and Principe', 'Saudi Arabia',
                    'Senegal', 'Serbia', 'Seychelles', 'Sierra Leone', 'Singapore', 'Slovakia', 'Slovenia',
                    'Solomon Islands', 'Somalia', 'South Africa', 'Spain', 'Sri Lanka', 'Sudan', 'Suriname',
                    'South Sudan', 'Svalbard', 'Swaziland', 'Sweden', 'Switzerland', 'Syrian Arab Republic', 'Taiwan',
    ...
    emergency_location_label = Label(New_Camp_Screen, text="Country Affected")
    emergency_location_label.pack()
    emergency_location_entry = ttk.Combobox(New_Camp_Screen, textvariable=area_affected, values=country_list)
    ...
    end_date_label.pack()
    Label(New_Camp_Screen, text="If the emergency has not finished yet, please leave the below boxes blank.").pack()
    ...
    def CreateNewCampSummary():
        ...
        New_Camp_StartDate_Summary_Label = Label(New_Camp_Summary_Screen, text="The start date of the new emergency is: %s" % (generate_start_date()))
        New_Camp_StartDate_Summary_Label.pack()

        New_Camp_EndDate_Summary_Label = Label(New_Camp_Summary_Screen, text="The end date of the new emergency is: %s" % (generate_end_date()))
        New_Camp_EndDate_Summary_Label.pack()

        New_Camp_Status_Summary_Label = Label(New_Camp_Summary_Screen, text="The status of the new emergency is: %s" % (generate_status()))
        ...
        def SubmitEmergency():
            ...
            new_emergency[5] = str(generate_start_date())
            new_emergency[6] = str(generate_end_date())
            new_emergency[7] = generate_status()

            new_emergency_string = '%'.join(new_emergency)

            emergency_database_file_append = open("emergency_database.txt", "a")
            emergency_database_file_append.write("%s" % new_emergency_string)
